		...
		switch(state) {
			case 0:
				...
				/* Begin Numerics and Identifiers */
					  ...
					  && line[currentChar] <= '9') {		state = 61;		//NUMERIC
					  	tempString += line[currentChar];						}
				else if(line[currentChar] == '-') {		state = 63;		//MINUS or NUMERIC
					  	tempString += line[currentChar];						}				
				else if(line[currentChar] >= 'a'
					  && line[currentChar] <= 'z') {		state = 64;		//IDENTIFIER
					  	tempString += line[currentChar];						}					  
...
//				else if(line[currentChar] == '\n')		currentChar = 79;					  //SPACEBAR, ADVANCE
			...
			break;
			case 2:
				if(line[currentChar] == 'D') {        	 state = 1224;   //END_TOKEN_ID
			...
			break;
			case 4:
				if(line[currentChar] == 'E') {           state = 1224;  //ELSE_TOKEN_ID
			...
			break;
			case 5:
				if(line[currentChar] == 'F') {        	 state = 1224;   //IF_TOKEN_ID
			...
			break;
			case 10:
				if(line[currentChar] == 'R') {        	 state = 1224;   //integer
					tempString = "integer";
					iString = "INTEGER";
				}
				else		 										 state = 1234;   //RESERVED WORD ERROR 
			...
			break;		
			case 12:
				if(line[currentChar] == 'T') {        	 state = 1224;   //INPUT_TOKEN_ID
			...
			break;
			case 20:
				if(line[currentChar] == 'E') {        	state = 1224;    //PROCEDURE_TOKEN_ID
			...
			break;      
			case 23:
				if(line[currentChar] == 'M') {			state = 1224;	 //PROGRAM_TOKEN_ID
			...
			break;			
			case 24:
				if(line[currentChar] == 'O') {        	state = 1224;   //DO_TOKEN_ID
			...
			break;
			case 29:
				if(line[currentChar] == 'E') {        	state = 1224;   //DECLARE_TOKEN_ID
			...
			break;		
			case 32:	
				if(line[currentChar] == 'L') {        	state = 1224;   //CALL_TOKEN_ID
			...
			break;		
			case 36:
				if(line[currentChar] == 'E') {        	state = 1224;   //VALUE_TOKEN_ID
			...
			break;
			case 39:
				if(line[currentChar] == 'L') {        	state = 1224;   //REAL
			...
			break;						
			case 45:
				if(line[currentChar] == 'E') 	{			state = 1224;   //REFERENCE_TOKEN_ID
			...
			break;			
			case 48:
				if(line[currentChar] == 'N') {        	state = 1224;   //MAIN_TOKEN_ID
			...
			break;									
			case 53:
				if(line[currentChar] == 'T') {        	state = 1224;   //OUTPUT_TOKEN_ID
			...
			break;									
			case 56:
				if(line[currentChar] == 'N') {			state = 1224;   //THEN_TOKEN_ID
			...
			break;
			case 60:
				if(line[currentChar] == 'E') {			state = 1224;   //WHILE_TOKEN_ID
		   ...
		   case 61:																  //NUMERIC
				...
				else if(line[currentChar] == '.') {	  state = 62;	//REAL
					  	tempString += line[currentChar];					}					
				else {
			...
			break;
			case 78:
				if(line[currentChar] == '/') {				state = 0;  //END COMMENT
					if((unsigned)(currentChar) < strlen(line))
			...
			/* BEGIN STRING STATES */
			case 79:
				if(line[currentChar] == '"') {				state = 0; //END QUOTATION
					tempString = "";
