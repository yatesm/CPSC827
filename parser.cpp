	...
	while(true) {
		...
		if(fPrecIter->second < gPrecIter->second) {
			if(cData->getFlag(9) == 1)
				cout<<"\t\t"<<setw(11)<<syntaxStack.top()->getTokenIdentifier()<<"\t<\t"<<setw(11)<<inputToken->getTokenIdentifier()<<endl;
			...
			if(cData->getFlag(9) == 1)
				cout<<"\t\t"<<setw(11)<<syntaxStack.top()->getTokenIdentifier()<<"\t=\t"<<inputToken->getTokenIdentifier()<<endl;		
			syntaxStack.push(inputToken);
			veryTemp = inputToken->getTokenString().find("TOKEN_ID");
			if(veryTemp != string::npos)
				semanticsStack.push(inputToken->getTokenIdentifier());
			...
			while(true) {
				...
				if(fPrecIter->second == gPrecIter->second) {				
					if(cData->getFlag(9) == 1) {
						if(fPrecIter->second < gPrecIter->second)
							cout<<"\t\t"<<setw(11)<<syntaxStack.top()->getTokenIdentifier()<<"\t<\t"<<setw(11)<<handleToken->getTokenIdentifier()<<endl;	
						else
							cout<<"\t\t"<<setw(11)<<syntaxStack.top()->getTokenIdentifier()<<"\t=\t"<<handleToken->getTokenIdentifier()<<endl;	
					}
					stackToken = syntaxStack.top();
					
//					delete syntaxStack.top();
					syntaxStack.pop();
				   ...
				   multimap<string, Production>::iterator it = productionMap.find(handle);
					...
					if(cData->getFlag(7) == 1) {
					    if(it != productionMap.end())
					        cout<<"\tReduction "<<it->second.getNum()<<".\t"<<it->second.getLHS()<<" --> "<<it->first<<endl;
					}
					...
					/*else {
					    cout<<endl<<"Stackability Error!"<<endl;
					    cout<<"START TO PANIC!"<<endl;
					    panic(it->second.getLHS());
				    }*/
